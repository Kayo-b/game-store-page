...
This game store page is inspired by the Steam store design but with a more traditional e-commerce layout.
...
#### Technical notes:
    ...
    This project allowed me to effectively apply most of the React concepts that I’ve learned so far. It was also my introduction to React Routes, which I now feel quite comfortable with. Handling all the async functions and making sure the values would be updated during render was challenging at times since this is my second time using an API and my first time using nested APIs inside a loop that needed to be within a `Promise.all()` method so that all fetches would be synced. I also learned a great deal about CSS during this project, mainly on the animation side of things using `@keyframes`.


    ![preview](./src/game-store-rec.gif)
...
#### Credits:
    ...
    - CheapShark and Steam for providing the free APIs.
    - Midjourney and FromSoftware for creating the artwork.